...
class _VehicleDetailScreenState extends State<VehicleDetailScreen> {
  ...
  Future<void> _loadExpenses() async {
    ...
    try {
      _expenses = await _expenseService.getExpenses(vehicleId);
      _totalExpenses = await _expenseService.getTotalExpenses(vehicleId);
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _sellVehicle() async {
    ...
    if (vehicleId == null) {
      _uyariGoster(
        'Araç kaydı bulunamadı. Lütfen listeyi yenileyip tekrar deneyin.',
      );
      return;
    }
                ...
                children: [
                  ...
                  InkWell(
                    ...
                    child: InputDecorator(
                      ...
                      child: Text(
                        _dateFormat.format(satisTarihi),
                        style: GoogleFonts.inter(
                          fontSize: 14,
                          color: AppTheme.textPrimary,
                        ),
  ...
  Future<void> _addExpense() async {
    ...
    if (vehicleId == null || userId == null) {
      _uyariGoster(
        'Oturum veya araç kaydı bulunamadı. Lütfen tekrar giriş yapın.',
      );
      return;
    }
  ...
  Future<void> _editExpense(Expense expense) async {
    final tutarController = TextEditingController(
      text: expense.tutar.toString(),
    );
    final aciklamaController = TextEditingController(
      text: expense.aciklama ?? '',
    );
  ...
  void _gecersizTutarUyari() {
    _uyariGoster('0’dan büyük geçerli bir tutar girin (örn. 150000).');
  }
  ...
  void _uyariGoster(String mesaj) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(mesaj, style: GoogleFonts.inter(fontSize: 13)),
  ...
  Widget _buildMobileBody(bool isSold) {
    return Column(
      children: [
        ...
        if (!isSold) ...[const SizedBox(height: 12), _buildSellButton()],
      ...
      children: [
        ...
        Row(
          children: [
            ...
            if (_vehicle.renk != null)
              Expanded(
                child: _heroStat(Icons.palette_outlined, _vehicle.renk!),
              ),
            if (_vehicle.kilometre != null)
              Expanded(
                child: _heroStat(
    ...
    return _card(
      ...
      child: Column(
        children: [
          ...
          _row('Durum', '', valueWidget: _statusBadge(_vehicle.durum)),
          if (hasSatici) ...[
            const SizedBox(height: 8),
            _sectionDivider(
              'Alındığı Kişi (Satıcı)',
              Icons.person_outline_rounded,
  ...
  Widget _buildFinanceCard(bool isSold) {
    ...
    return _card(
      ...
      child: Column(
        children: [
          _row('Alış Fiyatı', _currencyFormat.format(_vehicle.alisFiyati)),
          if (_vehicle.alisTarihi != null)
            _row('Alış Tarihi', _dateFormat.format(_vehicle.alisTarihi!)),
          if (isSold && _vehicle.satisFiyati != null)
            _row('Satış Fiyatı', _currencyFormat.format(_vehicle.satisFiyati)),
          if (_vehicle.satisTarihi != null)
            _row('Satış Tarihi', _dateFormat.format(_vehicle.satisTarihi!)),
          ...
          if (isSold && hasAlici) ...[
            const SizedBox(height: 8),
            _sectionDivider(
              'Satıldığı Kişi (Alıcı)',
              Icons.person_rounded,
              const Color(0xFF059669),
              const Color(0xFFD1FAE5),
            ),
            if (_vehicle.aliciAdi != null) _row('Ad Soyad', _vehicle.aliciAdi!),
  ...
  Future<void> _toggleRezerve() async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () => Navigator.pop(ctx, true),
            style: FilledButton.styleFrom(
              backgroundColor: isRezerve ? AppTheme.primary : AppTheme.warning,
              shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(8),
              ),
            ),
    ...
    if (confirm == true) {
      try {
        final updated = await _vehicleService.updateStatus(vehicleId, newDurum);
        setState(() => _vehicle = updated);
      } catch (e) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                AppError.from(e, ctx: 'Araç durumu güncellenemedi'),
              ),
              backgroundColor: AppTheme.error,
              behavior: SnackBarBehavior.floating,
              shape: RoundedRectangleBorder(
      ...
      children: [
        ...
        SizedBox(
          ...
          child: OutlinedButton.icon(
            ...
            icon: Icon(
              isRezerve ? Icons.undo_rounded : Icons.schedule_rounded,
              size: 17,
            ),
            label: Text(
        ...
        child: Row(
          children: [
            Container(
              ...
              child: Icon(_getExpenseIcon(e.tur), size: 16, color: ikonRenk),
            ),
            const SizedBox(width: 10),
            Expanded(
              child: Column(
            ...
            IconButton(
              ...
              onPressed: () async {
                ...
                if (expenseId == null) {
                  _uyariGoster(
                    'Gider kaydı bulunamadı. Lütfen sayfayı yenileyin.',
                  );
                  return;
                }
                try {
                  ...
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          AppError.from(err, ctx: 'Gider silinemedi'),
                        ),
                        backgroundColor: AppTheme.error,
                        behavior: SnackBarBehavior.floating,
                        shape: RoundedRectangleBorder(
  ...
  Color _getExpenseColor(String tur) {
    switch (tur) {
      case 'bakim':
        return const Color(0xFF0891B2); // cyan-600  — mekanik
      case 'boya':
        return const Color(0xFF7C3AED); // violet-600 — estetik
      case 'sigorta':
        return const Color(0xFF059669); // green-600  — güvence
      case 'vergi':
        return const Color(0xFFD97706); // amber-600  — resmi
      default:
        return const Color(0xFF64748B); // slate-500  — diğer
    }
  }
